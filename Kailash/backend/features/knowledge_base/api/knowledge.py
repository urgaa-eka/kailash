...
async def trigger_learning(current_user: dict = Depends(get_current_user)):
    ...
    from backend.features.eka_brain.jobs.daily_learning import async_daily_learning_pipeline
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to start pipeline: {str(e)}") from e
