...
from backend.features.auth.schemas import Token, UserLogin, UserRegister
from backend.features.users.models import User
from backend.platform.core.config import settings
from backend.platform.core.mongodb import get_db
from backend.platform.core.permissions import UserRole
from backend.platform.core.security import (
    create_access_token,
    decode_token,
    get_password_hash,
    verify_password,
)
from backend.platform.deps import get_current_active_user
from backend.platform.email_service import email_service
...
async def login(credentials: UserLogin, request: Request):
    ...
    try:
        ...
        if credentials.password and not verify_password(
...
async def get_user_permissions(current_user: User = Depends(get_current_active_user)):
    ...
    from backend.platform.core.permissions import ROLE_PERMISSIONS
    try:
