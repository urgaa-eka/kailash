...
from backend.features.eka_brain.agents.c5_multimodel_strategy import (
    classify_query_complexity,
    get_fallback_model,
    get_model_for_agent,
)
from backend.features.eka_brain.agents.prompts import (
    GANESHA_SYSTEM_PROMPT,
    get_agent_prompt,
    route_to_agent,
)
from backend.features.eka_brain.services.rag_service import get_rag_service
...
class GaneshaOrchestratorV2:
    ...
    async def process_request(
        ...
        try:
            ...
            product_context = user_context.get('product')
...
"""
    ...
    def _should_use_emergent(self, user_message: str) -> bool:
        ...
        # If it's an info request and not also a build request, don't use Emergent
        if (any(kw in message_lower for kw in info_keywords)
                and not any(kw in message_lower for kw in code_keywords)):
            return False
...
Create the prompt now."""
    ...
    def get_available_agents(self) -> list[dict]:
        ...
        from backend.features.eka_brain.agents.prompts import list_all_agents
