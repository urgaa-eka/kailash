...
def _reference_cards() -> str:
    return f"""
<div class="card"><h2>The five financial years</h2>
  <table><tr><th>FY</th><th>Audit status</th><th>Posture</th><th>Note</th></tr>{_years_rows()}</table>
...
def _flags_html(fin: FYFinancials, fy: str) -> str:
    ...
    return (f"<div class='card'><h2>Flagged for FY {_esc(fy)} — "
...
def _fy_panel(fin: FYFinancials, fy: str) -> str:
    ...
    p, s, t = fin.purchase, fin.sales, fin.tax
    return f"""
<div class="kpis">{_kpi_html(fin)}</div>
{_flags_html(fin, fy)}
<div class="grid2">
  <div class="card"><h2>Dept 2 · Vendor / Workshop Settlement — Net Payable</h2>
    <table><tr><th>Line</th><th class="num">FY {_esc(fy)}</th></tr>{_vendor_rows(p)}</table>
...
_STYLE = """
  ...
  * { box-sizing:border-box; margin:0; }
  body { background:var(--bg); color:var(--ink);
         font:14px/1.5 'IBM Plex Sans',-apple-system,'Segoe UI',Roboto,sans-serif;
         padding:24px; -webkit-font-smoothing:antialiased; }
  h1 { font-size:20px; font-weight:600; letter-spacing:-0.01em; }
  h2 { font-size:13px; margin-bottom:10px; color:var(--accent); font-weight:600;
       text-transform:uppercase; letter-spacing:0.04em; }
  .num, .kpi-value, tr.tot td, .mono {
...
def _shell(title: str, header: str, body: str) -> str:
...
def render_fy(provider: FinancialDataProvider, fy: str, *, today: date | None = None) -> str:
    ...
    options = "".join(
        ...
        for m in _model.FINANCIAL_YEARS)
    header = f"""<p class="sub">
  CIN {_esc(_model.ENTITY['cin'])} · PAN {_esc(_model.ENTITY['pan'])} · GSTINs: {_gstins()}<br>
  Financial year <select onchange="location.href='?fy='+this.value">{options}</select>
  · {conn_pill} <span class="dim">{_esc(provider.source_label())}</span>
  · generated {today.isoformat()}</p>"""
    body = _fy_panel(fin, fy) + _reference_cards()
    return _shell(f"Go4Garage · Financial Controller · FY {fy}", header, body)
...
def render_static(provider: FinancialDataProvider, *, today: date | None = None,
    ...
    header = f"""<p class="sub">
  CIN {_esc(_model.ENTITY['cin'])} · PAN {_esc(_model.ENTITY['pan'])} · GSTINs: {_gstins()}<br>
  Financial year <select id="fysel" onchange="showFy(this.value)">{options}</select>
  · {conn_pill} <span class="dim">{_esc(provider.source_label())}</span>
  · generated {today.isoformat()}</p>"""

    panels = "".join(
        f"<div class='fy-panel' data-fy='{_esc(fy)}'>{_fy_panel(provider.fy_financials(fy), fy)}</div>"
        for fy in fys)
    script = """
<script>
function showFy(fy){
  document.querySelectorAll('.fy-panel').forEach(function(p){
    p.style.display = (p.dataset.fy === fy) ? '' : 'none';
  });
}
document.addEventListener('DOMContentLoaded', function(){
  showFy(document.getElementById('fysel').value);
    ...
    body = panels + _reference_cards() + script
    return _shell("Go4Garage · Financial Controller", header, body)
