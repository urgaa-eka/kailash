...
- [x] 1. Verification foundation: shared contract, test harness, CI job

  - [x] 1.1 Create the `scripts/verify` package and shared contract
  ...
  - [x] 1.2 Build the test harness the checks are tested through
  ...
  - [x] 1.3 Add the `verify-scripts` job to `ci.yml`
...
- [x] 2. Toolchain prerequisite and CI failure un-masking

  - [x] 2.1 Make yarn available locally without changing the lockfile
  ...
  - [x] 2.2 Un-mask the `ci.yml` backend job and fix what surfaces
  ...
  - [x] 2.3 Un-mask the `ci.yml` frontend job and fix what surfaces
  ...
  - [x] 2.4 Un-mask the `deploy-backend.yml` test job and fix what surfaces
  ...
  - [x] 2.5 Checkpoint - all three gating jobs can fail, and pass
    - Ensure all tests pass, ask the user if questions arise.

- [x] 3. Reconcile repository state and resolve the open questions

  - [x] 3.1 Reconcile uncommitted Deployment_Critical_Path changes `[operator]`
  ...
  - [x] 3.2 Resolve whether production is intended to run the profiled services `[operator]`
  ...
  - [x] 3.3 Resolve the backend health path by observation `[live-docker]`
...
- [x] 4. Requirement 1 - container health diagnosis and remediation

  - [x] 4.1 Build the Phase A evidence collector `[live-docker]`
  ...
  - [x] 4.2 Build the pure classifier, attribution, and Diagnosis_Record emission
  ...
  - [x] 4.3 Implement the start-period rule
  ...
  - [x] 4.4 Run Phase C replay and Phase D executable evidence `[live-docker]`
  ...
  - [x] 4.5 Run Phase B uniform-cause probes and record the H1 verdict `[live-docker]`
  ...
  - [x] 4.6 Give the `frontend` container a health probe `[live-docker]`
  ...
  - [x] 4.7 Apply the probe corrections the diagnosis attributes `[live-docker]`
  ...
  - [x] 4.8 Ensure `/api/health` is served, per the task 3.3 finding
  ...
  - [x] 4.9 Add the live-stack health integration tests `[live-docker]`
  ...
  - [x] 4.10 Checkpoint - diagnosis recorded, H1 verdict written, stack healthy
    - Ensure all tests pass, ask the user if questions arise.

- [x] 5. Requirement 3 - configuration drift check

  - [x] 5.1 Implement `config_drift.py` with the project-id and repo-slug rules
  ...
  - [x] 5.2 Implement the `firebase-app-identity` rule
  ...
  - [x] 5.3 Assert the drift check exits 0 against the real repository
  ...
  - [x] 5.4 Add the `config-drift` job to `ci.yml`
  ...
  - [x]* 5.5 Add the `node-version` rule and pin CI's frontend Node to 20
...
- [x] 6. Requirement 4 - credential handling and secret scan

  - [x] 6.1 Implement `secret_scan.py`
  ...
  - [x] 6.2 Add the `secret-scan` job to `ci.yml`
  ...
  - [x] 6.3 Make credentials strictly required in compose
  ...
  - [x] 6.4 Add the unset-credential integration test `[live-docker]`
...
- [x] 7. Requirement 9 - repository state integrity

  - [x] 7.1 Implement `repo_state.py`
  ...
  - [x] 7.2 Add the destructive-operation guard to `deploy.sh`
  ...
  - [x] 7.3 Add the critical-path commit confirmation hook
...
- [x] 8. Requirement 5 - buildable build definitions

  - [x] 8.1 Implement `build_audit.py`
  ...
  - [x] 8.2 Remove the nine dead per-service Dockerfiles
  ...
  - [x] 8.3 Fix the `compose-build` job to build what it claims to build
  ...
  - [x] 8.4 Correct `docs/guides/docker-and-mcp.md` to describe the real build mechanism
  ...
  - [x] 8.5 Checkpoint - every tracked build file builds, drift and secret gates are live
    - Ensure all tests pass, ask the user if questions arise.

- [x] 9. Requirement 2 - deployment verification check

  - [x] 9.1 Implement `deployment_check.py` status and content-type rules
  ...
  - [x] 9.2 Implement the asset-manifest containment rule
  ...
  - [x] 9.3 Implement the certificate margin rule
  ...
  - [x] 9.4 Run the check against live production `[live-network]`
...
- [ ] 10. Requirement 6 - rollback mechanics
  ...
  - [x] 10.1 Report the deployed commit in the health response
  ...
  - [x] 10.2 Implement the `rollback.py` precondition check
  ...
  - [x] 10.3 Write the Rollback_Procedure runbook
  ...
  - [x] 10.4 Tag deployed backend images by commit SHA
...
- [x] 11. Workflow topology - gates and CI linkage

  - [x] 11.1 Implement `workflow_gate.py`
  ...
  - [x] 11.2 Make `ci.yml` callable as a gate
  ...
  - [x] 11.3 Add `preflight`, `ci-gate` and `verify-production` to `deploy-frontend.yml`
  ...
  - [x] 11.4 Add `preflight`, `ci-gate` and `verify-production` to `deploy-backend.yml`
...
- [ ] 12. Requirement 8 - staging environment and promotion gate
  ...
  - [x] 12.1 Confirm or flip the backend staging option from the H1 verdict `[operator]`
  ...
  - [x] 12.2 Configure the named Firebase staging channel
  ...
  - [x] 12.3 Create the staging compose overlay
  ...
  - [x] 12.4 Declare the staging credential set under distinct secret names
    - Create `deploy/staging/.env.staging.example` and declare the `STAGING_*` secret names in the workflows: `STAGING_POSTGRES_PASSWORD`, `STAGING_REDIS_PASSWORD`, `STAGING_PLATFORM_INTERNAL_TOKEN`, `STAGING_VULTR_HOST`, `STAGING_VULTR_USER`, `STAGING_VULTR_SSH_KEY` (~~`STAGING_FIREBASE_SERVICE_ACCOUNT`~~ — superseded: Firebase deploys use Workload Identity Federation, no key secret exists; org policy blocks SA key creation)
  ...
  - [x] 12.6 Add `deploy-staging` and `verify-staging` to both deploy workflows
    - In `deploy-frontend.yml`: a `deploy-staging` job deploying to the named channel (authenticating via Workload Identity Federation — the originally planned `STAGING_FIREBASE_SERVICE_ACCOUNT` secret was superseded), then `verify-staging` running `deployment_check.py --env staging`, then make `build-and-deploy` (production) `needs: [preflight, ci-gate, deploy-staging, verify-staging]`
  ...
  - [x] 12.7 Add the staging isolation rule to `workflow_gate.py`
  ...
  - [x] 12.8 Write the staging runbook
  ...
  - [x] 12.9 Enforce the workflow graph in CI
  ...
  - [x] 12.10 Checkpoint - production is reachable only through staging
    - Ensure all tests pass, ask the user if questions arise.

- [ ] 13. Requirement 7 - Docker host cleanup

  - [x] 13.1 Implement `orphan_review.py` and the retained-container schema
  ...
  - [x] 13.2 Implement the removal confirmation gate
  ...
  - [x] 13.3 Classify the six named containers from evidence `[live-docker]`
